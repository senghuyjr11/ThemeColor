...
public class MainActivity extends AppCompatActivity implements Conf.OnThemeSet {
    // Store the binding
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(preferenceDelegator.contains("SET_THEME")){
            setTheme(preferenceDelegator.getInt("SET_THEME"));
        }else {
        ...
        initView();
        initColor();
        Conf.onCheckTheme(getApplicationContext(), this);
    }
    ...
    public void onSetTheme(Context context, int color) {
        switch (color){
            case R.color.color_000000:
//                binding.llView.setBackground();
//                actionBarImgLeft.setImageResource(R.drawable.selector_ic_close_white);
                break;
            default:
                break;
        }
